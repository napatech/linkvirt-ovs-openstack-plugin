...
class LV_BaseOVS(object):
    ...
    def create_ovs_vif_port(self, bridge, dev, iface_id, mac, instance_id,
                            ...
                            pf_pci=None, vf_num=None, network_type=None):
        """Create OVS port
        ...
        if network_type == 'vxlan':
            col_values.append(('options',
                               {'tunnel-offload': network_type}))
        if tag:
            col_values.append(('tag', tag))
        if (interface_type == constants.OVS_DPDK_INTERFACE_TYPE
            and pf_pci and vf_num):
            devargs_string = "{PF_PCI},representor=[{VF_NUM}]".format(
                PF_PCI=pf_pci, VF_NUM=vf_num)
            col_values.append(('options',
                              {'dpdk-devargs': devargs_string}))
        elif (interface_type == constants.OVS_DPDK_INTERFACE_TYPE
              and pf_pci and vf_num is None):
            devargs_string = "{PF_PCI}".format(PF_PCI=pf_pci)
            col_values.append(('options',
                              {'dpdk-devargs': devargs_string}))
