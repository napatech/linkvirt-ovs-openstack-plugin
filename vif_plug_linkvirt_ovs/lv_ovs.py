...
from vif_plug_linkvirt_ovs.ovsdb import lv_ovsdb_lib
from vif_plug_ovs import linux_net


class LV_OvsPlugin(plugin.PluginBase):
    ...
    def __init__(self, config):
        super(LV_OvsPlugin, self).__init__(config)
        self.ovsdb = lv_ovsdb_lib.LV_BaseOVS(self.config)

    @staticmethod
    def gen_port_name(prefix, id):
        return ("%s%s" % (prefix, id))[:LV_OvsPlugin.NIC_NAME_LEN]

    @staticmethod
    def get_veth_pair_names(vif):
        return (LV_OvsPlugin.gen_port_name("qvb", vif.id),
                LV_OvsPlugin.gen_port_name("qvo", vif.id))
    ...
    def _plug_vhostuser(self, vif, instance_info):
        ...
        vif_name = LV_OvsPlugin.gen_port_name(
